...
export const getUserById = (req, res) => {
  ...
  if (!user) return res.status(404).json({ message: "usero no encontrado" });
...
export const updateUser = (req, res) => {
  ...
  if (index === -1) return res.status(404).json({ message: "usero no encontrado" });
...
export const deleteUser = (req, res) => {
  ...
  if (users.length === filteredUser.length) return res.status(404).json({ message: "usero no encontrado" });
  
  fs.writeFileSync(dataPath, JSON.stringify(filteredUser, null, 2));
  res.json({ message: "usero eliminado" });
};